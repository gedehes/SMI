...
# En-tête HTTP pour imiter un navigateur web et éviter le blocage 403 sur Slickcharts / Wikipédia
HEADERS = {
    'User-Agent': (
...
# --- RÉCUPÉRATION AUTOMATIQUE DES TICKERS (SLICKCHARTS -> WIKIPÉDIA -> FALLBACK) ---
@st.cache_data(ttl=86400)
def obtenir_sp500_tickers():
  """Récupère les ~500 actions du S&P 500 depuis Slickcharts, sinon Wikipédia, sinon liste fixe."""
...
def obtenir_nasdaq100_tickers():
  """Récupère les ~100 actions du NASDAQ 100 depuis Slickcharts, sinon Wikipédia, sinon liste fixe."""
  # 1. Tentative sur Slickcharts
  try:
...
# --- ONGLET 3 : MARKET BREADTH (SLICKCHARTS + WIKIPEDIA + FALLBACK) ---
with tab3:
  ...
  if st.button(
    ...
    with st.spinner(
        "Extraction des listes Slickcharts / Wikipédia et calcul du Market"
        " Breadth..."
    ):
